...
class UserModel(object):
    ...
    def add_sample(self, mp4, thresh=.08):
        ...
        while True:
            ...
            if(
                ...
                self.stat_dict['base_time'] = time_stamp
                self.stat_dict['left_base_elbow_angle'] = \
                self.left_elbow_angle(joints) 
                self.stat_dict['right_base_elbow_angle'] = \
                ...
                base_met and 
                not release_met and
                self.compare_joints(joints, release_joints) <= 220 and #205
                self.compare_elbows(joints, release_joints) <= 70 #60
                ):
                release_met = True

                self.stat_dict['release_time'] = time_stamp
                self.stat_dict['rise_length'] = self.stat_dict['release_time'] - self.stat_dict['base_time']
                self.stat_dict['left_release_elbow_angle'] = \
                self.left_elbow_angle(joints) 
                self.stat_dict['right_release_elbow_angle'] = \
                        self.right_elbow_angle(joints)
        ...
        self.stat_dict['shot_length'] = time_stamp

        # Update user metadata, given this sample
        self.average_stats(self.stat_dict)
    ...
    def right_elbow_angle(self, j):
        '''
            Given the skeleton's joint dictionary,
            computes the angle of the skeleton's right elbow
        '''
        pointA = np.array(j[2])
        pointB = np.array(j[3])
        pointC = np.array(j[4])
