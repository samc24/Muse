...
input_dir = 'basketball_photos/release/*'# sys.argv[1] + '*'
output_name = 'release_output'# input_dir.split('/')[2] + input_dir.split('/')[3][:-8]
...
for f in iglob(input_dir):
    ...
    # hasFrame = True 
    frame = cv2.imread(input_source)# cap = cv2.VideoCapture(input_source)
    ...
    for i in range(nPoints):
        ...
        if prob > threshold:
            cv2.circle(frameCopy, (int(x), int(y)), 8, (255, 0, 255), thickness=-1, lineType=cv2.FILLED)
            cv2.putText(frameCopy, "{}".format(i), (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2,
