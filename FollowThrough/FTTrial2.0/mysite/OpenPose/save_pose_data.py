...
if len(sys.argv) < 2:
    ...
    outfile_path = 'csv/sample_video.csv'
    djangoDir = ""
    outputFN = "output.mp4"
else:
    test = False
    ...
    outfile_path = 'csv/'+sys.argv[1][:-4]+'.csv'
...
cap = cv2.VideoCapture(djangoDir+"uploads/"+input_source)
...
df.to_csv(djangoDir+outfile_path, index = False)
print('save complete')

import smooth_pose_data
