...
**Status:** Approved 2026-04-24. Pre-implementation validation (one lightweight spike) pending.
**Scope:** Pick the architecture for the Follow Through mobile rewrite. v1 = iOS + Android phone app; v2 = watchOS companion.
**Decision owner:** Sameer (solo).

## Decision

**Native twins.** Jetpack Compose on Android + SwiftUI on iOS, two separate codebases, no KMP shared layer. Analysis logic is implemented twice (Kotlin + Swift) with behavior parity enforced mechanically: a `mobile/SHARED_SPEC.md` contract plus shared JSON test fixtures in `mobile/shared/fixtures/`. Both implementations run the same fixtures and assert the same outputs within float tolerance; tests pin the contract rather than a shared language.

watchOS is a first-class v2 target, living inside the iOS Xcode project as a watchOS app target, sharing a local Swift Package with the iOS phone app. WatchConnectivity carries phone-derived coaching events to the watch; the watch independently samples CoreMotion for parallel wrist-mechanics signal.
...
**Decision required:** pick the mobile architecture that preserves the legacy feature set, adds the demo's quantified analytics UX, runs on both iOS and Android, retires the server, remains maintainable by one engineer using AI-assisted coding, and leaves a clean path for a watchOS companion in v2.

## Goals

1. Ship to iOS + Android stores from a single project root (`Muse/mobile/`).
...
4. On-device pose via MediaPipe Tasks using the first-party SDKs (MediaPipe Tasks Android, MediaPipe Tasks iOS). No upload-to-server in the critical path.
5. Maintainable solo + AI-assisted. Architecture must keep code in AI's highest-fluency mobile patterns (idiomatic Compose, idiomatic SwiftUI, native SDKs) and out of its lowest-fluency mobile patterns (cross-language cinterop, multi-platform Gradle, generated Swift APIs).
6. watchOS v2 companion: haptic + glance live feedback during recording drills, parallel wrist-IMU signal, clean integration path.
...
4. **watchOS companion is on the 12-18 month roadmap.** Phone-to-watch feedback relay during drills + watch-IMU parallel signal. Architectures that cannot cleanly reach watchOS are disqualified.
...
| Watch v2: haptic live feedback during drill | New | Phone pose -> WatchConnectivity -> Core Haptics. |
| Watch v2: parallel wrist IMU signal | New | CoreMotion on watchOS, release detection + wrist snap angle. |
...
- watchOS v2 is not viable. Any watch feature becomes a standalone Swift target, which defeats Flutter's single-codebase argument precisely for the feature most likely to differentiate the product.
...
## Consequences of choosing E

- **Repo layout:** `Muse/mobile/android/` (Gradle project, Compose app), `Muse/mobile/ios/` (Xcode project with iOS and watchOS targets), `Muse/mobile/shared/` (SHARED_SPEC.md, JSON fixtures, design tokens). CI adds matrix jobs: Android (`ubuntu-latest`, Gradle) and iOS (`macos-latest`, Xcode). Python jobs untouched.
- **Seed data:** the 15 hardcoded pros from `Startup.java` become a `mobile/shared/pros_seed.json` fixture, loaded on first launch on each platform.
- **Analysis port:** `demo/analysis.py` -> `mobile/android/app/src/main/kotlin/analysis/Analysis.kt` + `mobile/ios/FollowThrough/Analysis.swift`. Ten pytest tests -> ten JUnit tests on Android + ten XCTest tests on iOS, all loading fixtures from `mobile/shared/fixtures/*.json`. Float tolerance pinned.
- **Pose pipeline:** MediaPipe Tasks Android SDK on the Kotlin side; MediaPipe Tasks iOS SDK on the Swift side. Both first-party; no cinterop.
- **watchOS v2:** lives inside the iOS Xcode project as a watchOS app target, shares a local Swift Package with iOS phone app (models, analysis contract, design tokens). WatchConnectivity for phone-derived feedback events (latency budget: 150ms). CoreMotion for watch-IMU wrist signal.
- **Backend:** none for v1. Bundled templates only. Supabase deferred to post-v1.
- **Server retirement:** `FTTrial2.0/` archived then deleted after mobile v1 ships. `WORK_NOTES.md` + `CLAUDE.md` updated.
- **Lifetime UI duplication:** ~600-900 LoC per platform, steady-state ~1.3-1.5x per new screen. Accepted.
...
The original draft listed three KMP-specific spikes (MediaPipe cinterop, Skie interop, two-AVPlayer sync). Two evaporate under E; one remains as a lightweight performance check, not a gate.

1. **Performance sanity check: MediaPipe Tasks iOS in pure Swift hits 30fps on iPhone 12+ for the 18-joint pose config.** Validate, not gate. SDK is first-party; this is confirming frame-rate, not discovering interop issues. Two-day prototype.

Two lightweight validations alongside:
- **AVPlayer dual-player sync.** Two `AVPlayer`s sharing a clock via `CADisplayLink`; drift stays under one frame over 15 seconds. Standard AVFoundation pattern; one afternoon.
- **WatchConnectivity feedback latency round-trip.** Phone sends an event; watch receives and fires haptic within 150ms end-to-end. Apple's published benchmark is 50-100ms; this is a sanity check on real-device conditions.

If performance sanity check passes, implementation commits. If it fails (unlikely for a first-party SDK), v1 falls back to record-then-analyse (no real-time live feedback); watch v2 then becomes strictly parallel-IMU-signal-only, not phone-relay.

## Open questions post-decision (not this doc's scope)

1. `SHARED_SPEC.md` format: prose + YAML function signatures, or typed markdown with JSON Schema fragments for inputs / outputs?
2. Test fixture authoring: capture from `demo/analysis.py` with a one-time script, or hand-write synthetic keypoint sequences for edge cases?
3. Android min SDK: API 26 (8.0) for CameraX + modern Compose baseline, or API 29 (10) for simpler permission model?
4. iOS min version: 17 (SwiftUI 5 maturity, watchOS 10) or 16 (broader install base)?
5. Watch v2 lands with v1.0 release or first minor release?
6. Shot overlay export: compute overlay at record time and cache video with baked-in overlay, or compute per-frame at playback and export via video-composition? Storage-vs-compute tradeoff; deferred.
